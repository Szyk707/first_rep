...
$stmt2 = $db->query("SELECT * FROM tasks_todo WHERE `id`='$id'");
...
if (isset($_SESSION['czyZalogowany']) && $_SESSION['czyZalogowany'] == true) {
    if ($task != null) {
        $db->query("UPDATE `tasks_todo` SET `task` = '$task' WHERE `id` = $id;");
...
<body>
    ...
    <h2 class="ms-3">Zmień swoje zadanie <?= $users['login'] ?></h2>
        ...
        <div class="row">
            <div class="col">
                <div class="input-group task_area m-0">
                    ...
                    <textarea name="task" class="form-control task_form" aria-label="With textarea"><?= $tasks['task'] ?></textarea>  
                </div>
            </div>
            <div class="col">
                <button type="submit" class="btn btn-success m-2">Gotowe</button>
            </div>
        </div>
